...
public class Select {
    ...
    private ArrayList<DescuentoEscalarBean> listaDescuentoEscalar = null;
    private ArrayList<DescuentoRegularBean> listaDescuentoRegular = null;
...
//					null);
    ...
    public ArrayList<DescuentoEscalarBean> listaDescuentoEscalar() {

        listaDescuentoEscalar = new ArrayList<DescuentoEscalarBean>();
        DescuentoEscalarBean modelCabecera;
        ArrayList<DescuentoEscalarDetalleBean> detalle;
        DescuentoEscalarDetalleBean modelDetalle;
        ...
        if (data.getCount() > 0) {
            while (data.moveToNext()) {
                modelCabecera = new DescuentoEscalarBean();
                modelCabecera.setCode(data.getString(data.getColumnIndex(contexto.getResources().getString(R.string.C_DESC_ESCALA_CODE))));
                modelCabecera.setName(data.getString(data.getColumnIndex(contexto.getResources().getString(R.string.C_DESC_ESCALA_NAME))));

                Cursor dataDetalle = db.rawQuery(" SELECT * FROM "
                        + contexto.getResources().getString(R.string.TD_DESCUENTO_ESCALA_1)
                        + " WHERE " + contexto.getResources().getString(R.string.C_DESC_ESCALA_1_CODE)
                        + " = '" + modelCabecera.getCode() + "'", null);

                if (dataDetalle.getCount() > 0) {
                    detalle = new ArrayList<DescuentoEscalarDetalleBean>();
                    while (dataDetalle.moveToNext()) {
                        modelDetalle = new DescuentoEscalarDetalleBean();
                        modelDetalle.setCode(dataDetalle.getString(data.getColumnIndex(contexto.getResources().getString(R.string.C_DESC_ESCALA_CODE))));

                        detalle.add(modelDetalle);
                    }
                    dataDetalle.close();
                    modelCabecera.setDetalle(detalle);
                }

                listaDescuentoEscalar.add(modelCabecera);
            }
